...
import * as studentService from '@/common/services/student';
...
export function setup() {
  ...
  async function fetch(
    options: Parameters<typeof studentService.getStudentsByGroup>[0]
  ) {
    const opts = {
    ...
    try {
      ...
      store.pageInfo = data.students?.pageInfo ?? null;
      store.totalCount = data.students?.totalCount ?? -1;
  ...
  async function newStudent(
    input: Parameters<typeof studentService.newStudent>[0]
  ) {
  ...
  async function getByAccountNumber(accountNumber: string): Promise<Student[]> {
    const data = await studentService.getStudentsByAccountNumber({
      accountNumber,
    });
