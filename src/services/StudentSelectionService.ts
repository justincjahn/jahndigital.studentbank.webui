...
export class StudentSelection extends Array<IStudentSelection> {
  ...
  getStudents(group: Group|null = null): Student[] {
    if (group !== null) {
      return this.filter((item) => item.type === StudentSelectionType.STUDENT
        && !item.excluded
    ...
    return this
      .filter((item) => item.type === StudentSelectionType.STUDENT && !item.excluded)
      .map((item) => item.object as Student);
  }
  ...
  getExcludedStudents(group: Group|null = null): Student[] {
    ...
    this.forEach((item) => {
      if (item.type === StudentSelectionType.GROUP) { return; }
      if (item.excluded === false) { return; }
      if (group !== null && (item.object as Student).groupId !== group.id) { return; }
  ...
  hasStudent(student: Student|null, implicit = true): boolean {
    ...
    const studentList = this.filter(
      (item) => item.type === StudentSelectionType.STUDENT
  ...
  toggleStudent(student: Student|null) {
    ...
    const excluded: IStudentSelection|undefined = this.find(
      (x) => x.type === StudentSelectionType.STUDENT
  ...
  async resolve(): Promise<Student[]> {
    ...
    this.filter((item) => item.excluded).forEach((item) => {
      if (item.type === StudentSelectionType.GROUP) {
        students = students.filter((filter) => filter.groupId !== item.object.id);
      } else {
